...
%><%
  ...
  if(username == null) username = "";
    ...
    if(param != null && !param.isEmpty() ){
      ...
      sql = "SELECT * FROM blog WHERE title LIKE \""+'%'+mo+'%'+"\" OR content LIKE \""+'%'+mo+'%'+"\" AND user='%s'";
    }
    else{
      String fmt = "select * from blog WHERE user ='%s' limit %d,%d";
      sql = String.format(fmt,username,pgno*pgcnt, pgcnt);
    }
    ...
    table.append("<table><tr><th>order</th><th>Title</th><th>User</th><th>-</th></tr>");
	  while(rs.next()) {
             table.append(String.format("<tr><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr>",
                    num,"<a href='readblog.jsp?pid="+rs.getString("id")+"&my=1'>"+rs.getString("title")+"</a>",rs.getString("user"),
                    "<a href='deleteblog.jsp?pid="+rs.getString("id")+"'>Delete</a>")
                    );
                    num++;
	  }
    table.append("</table>");
...
<body>
  <div class="container">
    <form action = "myblog.jsp" method="post" name="f">
    ...
    <div style = "float:right">
      <a href="myblog.jsp?pgno=<%=pgprev%>&pgcnt=<%=pgcnt%>"><%=Lastpage%></a>
      <a href="myblog.jsp?pgno=<%=pgnext%>&pgcnt=<%=pgcnt%>"><%=Nextpage%></a>
  </div>
  </div>
</body>
</html>
